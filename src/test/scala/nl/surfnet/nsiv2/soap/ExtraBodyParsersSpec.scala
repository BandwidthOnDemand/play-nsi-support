...
class ExtraBodyParsersSpec extends mutable.Specification
  ...
  class Fixture extends WithApplication() {
    ...
    implicit lazy val actionBuilder: ActionBuilder[Request, AnyContent] = controllerComponents.actionBuilder
  ...
  "SoapBodyParser" should {
    ...
    "give InternalServerError with a SOAP fault when the soap message is not valid" in new Fixture {
      val response = run(extraBodyParsers.soap(NsiXmlDocumentConversion).apply(FakeSoapRequest()).run(Source(List(ByteString("<nonSoap></nonSoap>")))))
    ...
    "give EntityTooLarge when the input is to large" in new Fixture {
      val response = run(extraBodyParsers.soap(NsiXmlDocumentConversion, 10).apply(FakeSoapRequest()).run(Source(List(ByteString("<test></test>")))))
    ...
    "give a SOAP message for a valid request" in new Fixture {
      val response = run(extraBodyParsers.soap(NsiXmlDocumentConversion).apply(FakeSoapRequest()).run(FileIO.fromPath(new File("src/test/resources/reserve.xml").toPath)))
  ...
  "NsiProviderParser" should {
    ...
    "give NSI Initial Reserve for a valid reserve request" in new Fixture {
      val response = run(extraBodyParsers.nsiProviderOperation.apply(FakeSoapRequest()).run(FileIO.fromPath(new File("src/test/resources/reserve.xml").toPath)))
    ...
    "give NSI Modify Reserve for a valid reserve request" in new Fixture {
      val response = run(extraBodyParsers.nsiProviderOperation.apply(FakeSoapRequest()).run(FileIO.fromPath(new File("src/test/resources/reserve_modify.xml").toPath)))
    ...
    "give InternalServerError when NSI Reserve contains extra xml" in new Fixture {
      val response = run(extraBodyParsers.nsiProviderOperation.apply(FakeSoapRequest()).run(FileIO.fromPath(new File("src/test/resources/reserve_additional_xml.xml").toPath)))

      response must beLeft.like {
        case result =>
          status(result) must beEqualTo(500)
          contentType(result) must beEqualTo(Some(SOAPConstants.SOAP_1_1_CONTENT_TYPE))
    ...
    "give InternalServerError when the NSI headers are missing" in new Fixture {
      val response = run(extraBodyParsers.nsiProviderOperation.apply(FakeSoapRequest()).run(FileIO.fromPath(new File("src/test/resources/reserve_without_headers.xml").toPath)))
    ...
    "give InternalServerError when there are multiple NSI headers" in new Fixture {
      val response = run(extraBodyParsers.nsiProviderOperation.apply(FakeSoapRequest()).run(FileIO.fromPath(new File("src/test/resources/reserve_with_duplicate_headers.xml").toPath)))
  ...
  "NsiRequesterParser" should {
    "give NSI Reserve Commit for a valid reserve confirmed request" in new Fixture {
      val response = run(extraBodyParsers.nsiRequesterOperation.apply(FakeSoapRequest()).run(FileIO.fromPath(new File("src/test/resources/reserveconfirmed.xml").toPath)))
